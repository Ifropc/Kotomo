...
class TestCleanOCR {
    var runRebuild = false

    @BeforeTest
    fun rebuild() {
        if (runRebuild) {
            Parameters.getInstance().cacheDir.deleteRecursively()
            Parameters.getInstance().cacheDir.mkdir()
    ...
    private fun testPoint(filename: String, expected: String, accpetedMarginPercent: Float) {
